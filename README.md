...
### 🧠 The Financial Health Index (FHI) Framework
...
| **3. Resilience to Shocks** | Buffers and psychological readiness against unexpected events (e.g., climate events, health crises, economic downturns). | `has_insurance` (and sub-types), `perception_insurance_important`, `attitude_stable_business`, `attitude_more_successful` | 
...
> **💡 Note on Structural Modifiers:** > Variables such as `country`, `owner_age`, `owner_sex`, and `business_age` are not direct components of the Financial Health Index. Instead, the models utilize these as **Structural Modifiers**—macro-level context that dictates *how* a business interacts with the 4 dimensions above. Furthermore, formalization metrics (like `keeps_financial_records` and `compliance_income_tax`) act as the critical bridge connecting informal SMEs to Dimension 4 (Formal Credit Networks).
...
| :--- | :--- | :--- |
| **Target Variable** | `Target` | The Financial Health Index tier (`Low`, `Medium`, `High`). |
...
| **Firmographics** | `keeps_financial_records` | Whether the business tracks finances (e.g., Yes, No, Sometimes, Missing). |
...
| **Insurance/Risk** | `has_insurance` | Master flag indicating whether the owner/business has any active insurance. |
| **Insurance/Risk** | `medical_insurance` *(etc)* | Specific types of insurance coverage possessed by the owner or business. |
| **Risk Perception** | `perception_insurance_important`| The owner's belief about the usefulness of insurance. |
| **Risk Perception** | `perception_cannot_afford` | Indicates cost as the main barrier to obtaining insurance. |
| **Psychological** | `attitude_stable_business` | Confidence in the stability of the local business environment. |
| **Psychological** | `attitude_satisfied_achievement`| Personal satisfaction with business progress to date. |
| **Psychological** | `attitude_more_successful` | Optimism about stronger business performance in the coming year. |
